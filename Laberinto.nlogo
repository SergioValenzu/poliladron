breed [ladrones ladron]; variables globales
...
;  ask turtles  [ ifelse [pcolor] of patch-ahead 1 = pink
;      [ lt random-float 90 ]   ;; We see a blue patch in front of us. Turn a random amount.
...
;   if [pcolor] of patch-ahead 0.1 = pink[
;    set heading (- heading)
;
;  ]
  ...
  ifelse color-delante = pink [
    ifelse getValor [right 90][left 30]
bounce
  ]
  [ forward 1 ]
...
to-report getValor ;para que no se cicle y el polici salgo del cuadro
  report random 2 = 1
end
@#$#@#$#@
GRAPHICS-WINDOW
